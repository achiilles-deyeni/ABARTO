const MachineryPart = require("../models/machines"); // Assuming model name based on routes
...
exports.searchMachinery = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, count: machines.length, data: machines });
  } catch (error) {
    console.error("Error searching machinery:", error);
    res
      .status(500)
      .json({ success: false, error: "Server error searching machinery" });
  }
};
