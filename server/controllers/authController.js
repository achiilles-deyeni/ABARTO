...
exports.loginAdmin = async (req, res) => {
  ...
  try {
    ...
    if (!admin) {
      return res
        .status(401)
        .json({ success: false, message: "Invalid credentials." }); // Use 401 Unauthorized
    }
    ...
    res
      .status(500)
      .json({ success: false, error: "Server error during admin login." });
  }
};
...
exports.loginUser = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    return res
      .status(401)
      .json({ success: false, error: "Invalid credentials" }); // Use 401 Unauthorized
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Error generating authentication token" });
  }
});
